...
(function (wid, dcm) {
	...
	function checkRange(year, month, cal) {
		...
		$id(cal.container + 'CalendarTitleLeft').classList.remove('disabled');
		$id(cal.container + 'CalendarTitleRight').classList.remove('disabled');
		if (cal.canViewDisabled) return;
		if (new Date(year, month).getTime() >= new Date(cal.endTime[0], cal.endTime[1] - 1).getTime())
			$id(cal.container + 'CalendarTitleRight').classList.add('disabled');
		if (new Date(year, month).getTime() <= new Date(cal.beginTime[0], cal.beginTime[1] - 1).getTime())
			$id(cal.container + 'CalendarTitleLeft').classList.add('disabled');
	}
	...
	function generateTitleMonth(idx, year, month, cal) {
		...
		return '<i>' +  monthArr[cal.monthType][new Date(year, month).getMonth()]
			+ '</i>' + new Date(year, month).getFullYear();
	}

	function generateItemTitle(cal) {
		var chinese       = '<span>周一</span><span>周二</span><span>周三</span><span>周四</span><span>周五</span><span>复习日</span>';
		var english       = '<span>M</span><span>T</span><span>W</span><span>T</span><span>F</span><span>S</span>';
		var chineseString = cal.isSundayFirst ? ('<span>考试日</span>' + chinese) : (chinese + '<span>考试日</span>');
	...
	function touchMove(event, cal) {
		...
		if (cal.canViewDisabled) {
			...
			if (cal.isToggleBtn) $id(cal.container + 'CalendarTitleRight').classList.add('disabled');
		} else if (offset > 0 && new Date(cal.currentYear, cal.currentMonth - 1).getTime() <= new Date(cal.beginTime[0], cal.beginTime[1] - 2).getTime()) {
			// 左滑到了零界点
			cal.isRangeChecked = true;
			if (cal.isToggleBtn) $id(cal.container + 'CalendarTitleLeft').classList.add('disabled');
		} else {
			cal.isRangeChecked = false;
			var movedis        = cal.distance + (offset - 0);
			transformFormat(cal.box, movedis);
	...
	Calendar.prototype = {
		...
		initDomFuc: function () {
			...
			html += _this.isToggleBtn ? '<span id="' + _this.container + 'CalendarTitleLeft" class="calendar-title-left">上一月</span>' +
			'<span id="' + _this.container + 'CalendarTitleRight" class="calendar-title-right">下一月</span>' : '';
		...
		initBinding: function () {
			...
			if (_this.isToggleBtn) {
				on('touchstart', _this.container + 'CalendarTitleLeft', function (e) {
					if(/disabled/.test(e.target.className))return;
				...
				on('touchstart', _this.container + 'CalendarTitleRight', function (e) {
					if(/disabled/.test(e.target.className))return;
					infinitePosition(_this);
					setTimeout(function () {
						_this.distance = _this.distance - _this.width;
